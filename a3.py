...
class GameApp:
    ...
    def __init__(self, master, task=MASTERS, dungeon_name="game3.txt"):
...
class AdvancedDungeonMap(DungeonMap):
    ...
    def draw_grid(self,dungeon,player_position):
        ...
        doorimg = doorimg.resize((s,s),Image.Resampling.LANCZOS)
        
        keyimg= Image.open('images/key.gif')
        keyimg= keyimg.resize((s,s),Image.Resampling.LANCZOS)
        
        wallimg= Image.open('images/wall.gif')
        wallimg = wallimg.resize((s,s),Image.Resampling.LANCZOS)
        
        moveimg= Image.open('images/moveIncrease.gif')
        moveimg = moveimg.resize((s,s),Image.Resampling.LANCZOS)
        
        plimg= Image.open('images/player.gif')
        plimg = plimg.resize((s,s),Image.Resampling.LANCZOS)
        
        spaceimg= Image.open('images/empty.gif')
        spaceimg = spaceimg.resize((s,s),Image.Resampling.LANCZOS)
...
class StatusBar(tk.Frame):
    ...
    def __init__(self,master,parent,moves):
        ...
        clockimg = clockimg.resize((75,75),Image.Resampling.LANCZOS)
        lightningimg= Image.open('images/lightning.gif')
        lightningimg = lightningimg.resize((75,75),Image.Resampling.LANCZOS)
...
class LivesStatusBar(tk.Frame):
    ...
    def __init__(self,master,parent):
        ...
        lifeimage = lifeimage.resize((75,75),Image.Resampling.LANCZOS)
